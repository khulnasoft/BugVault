...
from bugvault.models import Flaw, FlawSource, Impact
...
class TestFlawModelIntegration(object):
    ...
    def test_create_api(self, monkeypatch, auth_client, test_bugvault_api_uri):
        sync_count = 0
    ...
    def test_update_api(self, monkeypatch, auth_client, test_bugvault_api_uri):
        sync_count = 0
        ...
        flaw.save()
        response = auth_client().get(f"{test_bugvault_api_uri}/flaws/{flaw.uuid}")
        assert response.status_code == 200

        response = auth_client().put(
            f"{test_bugvault_api_uri}/flaws/{flaw.uuid}",
            {
                "uuid": flaw.uuid,
                "cve_id": flaw.cve_id,
    ...
    def test_create_jira_task_param(self, monkeypatch, auth_client, test_bugvault_api_uri):
        def mock_create_or_update_task(self, flaw):
            flaw.task_key = "TASK-123"
            return "TASK-123"
        ...
        flaw = Flaw.objects.get(uuid=flaw.uuid)
        response = auth_client().put(
            f"{test_bugvault_api_uri}/flaws/{flaw.uuid}?create_jira_task=1",
            {
                "uuid": flaw.uuid,
                "cve_id": flaw.cve_id,
                "title": f"{flaw.title} appended test title",
