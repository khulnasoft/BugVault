...
#***********************************
# NOTE: Useful if you need to rebuild the bugvault-service image.
...
apply-requirements-txt: check-reg check-venv sync-deps compose-up
	@echo ">appyling requirements.txt on bugvault-service"
	$(podman) exec -it bugvault-service pip3 install -r /opt/app-root/src/requirements.txt
	@echo ">appyling requirements.txt on bugvault_celery_1"
	$(podman) exec -it bugvault_celery_1 pip3 install -r /opt/app-root/src/requirements.txt
	@echo ">appyling requirements.txt on bugvault_celery_2" # if you have more celery replicas, you're on your own
	$(podman) exec -it bugvault_celery_2 pip3 install -r /opt/app-root/src/requirements.txt || true  # do not fail if only 1 host is configured
...
#***********************************
# NOTE external changes to the key file perms cause a hard-to-debug failure, therefore resetting permissions always even if it is not bugvault's fault.
