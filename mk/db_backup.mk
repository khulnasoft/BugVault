...
db-backup:
	...
	@echo "   podman exec -it bugvault-service python3 manage.py showmigrations"
	@echo "3. Make a note of the list of migrations included in the backup. Record with:"
	@echo "   make compose-up"
	@echo "   podman exec -it bugvault-service python3 manage.py showmigrations > bugvault_data_backup_dump_showmigrations.txt"
	@echo ""
	...
	@echo ">backing up DB to bugvault_data_backup_dump.db.gz"
	@make stop-local
	@make start-local-psql
	@$(podman) exec -it bugvault-data bash -c 'rm -f /var/lib/postgresql/data/bugvault_data_backup_dump.db.gz && pg_dump -Fp bugvault | gzip > /var/lib/postgresql/data/bugvault_data_backup_dump.db.gz && echo "pg_dump created inside container"'
	@$(podman) cp bugvault-data:/var/lib/postgresql/data/bugvault_data_backup_dump.db.gz bugvault_data_backup_dump.db.gz && echo "pg_dump backup copied from container"
	@$(podman) exec -it bugvault-data bash -c 'rm -f /var/lib/postgresql/data/bugvault_data_backup_dump.db.gz'
	@ls -la bugvault/migrations > bugvault_data_backup_dump_known_migrations.txt
	@git log -v --source --log-size --notes -n 1 > bugvault_data_backup_dump_git_commit_status.txt
	@echo -e "\n---\n" >> bugvault_data_backup_dump_git_commit_status.txt
	@git status >> bugvault_data_backup_dump_git_commit_status.txt
...
db-restore: stop-local start-local-psql
	@echo ">restoring bugvault_data_backup_dump.db.gz"
	@[ -f bugvault_data_backup_dump.db.gz ] || { echo "Error! bugvault_data_backup_dump.db.gz doesn't exist." ; exit 1 ; }
	@$(podman) cp bugvault_data_backup_dump.db.gz bugvault-data:/var/lib/postgresql/data/bugvault_data_backup_dump.db.gz.x && echo "backup dump copied to container"
	@$(podman) exec -it bugvault-data bash -c 'zcat /var/lib/postgresql/data/bugvault_data_backup_dump.db.gz.x | sed "s/^SELECT pg_catalog.set_config..search_path/-- &/" | gzip > /var/lib/postgresql/data/bugvault_data_backup_dump.db.gz && echo "search path fixed"'
	@$(podman) exec -it bugvault-data dropdb --if-exists -h bugvault-data -p 5432 bugvault && echo "existing database bugvault dropped"
	@sleep 1
	@$(podman) exec -it bugvault-data createdb  -T template0 bugvault && echo "created new database"
	@sleep 1
	@$(podman) exec -it bugvault-data bash -c 'zcat /var/lib/postgresql/data/bugvault_data_backup_dump.db.gz | psql bugvault && echo "dump restored inside container"'
	@$(podman) exec -it bugvault-data bash -c 'rm -f /var/lib/postgresql/data/bugvault_data_backup_dump.db.gz*'
	@sleep 3
	@$(podman) stop bugvault-data
...
.PHONY : db-drop
db-drop:
	@echo -n "This will delete data in the database. It will delete the bugvault_pg-data volume. As a side effect, it will also remove all bugvault containers. Are you sure? [y/N]" && read ans && [ $${ans:-N} = y ]
	$(podmancompose) -f docker-compose.yml -f docker-compose.test.yml down
	podman volume rm bugvault_pg-data -f


# NOTE: The following approach doesn't work. If a new db is created, celery crashes with [1]. If a new db is not created, bugvault-service crashes with [2]. It seems there's some inherent magic that auto-setups the db if the container and the volume are freshly created at the same time, and that magic is not run when the db is simply dropped or dropped & recreated.
# .PHONY : db-drop
# db-drop: stop-local start-local-psql
# 	@echo -n "This will delete data in the database. Are you sure? [y/N]" && read ans && [ $${ans:-N} = y ]
# 	@$(podman) exec -it bugvault-data dropdb --if-exists -h bugvault-data -p 5432 -U bugvault_app_user bugvault && echo "existing database bugvault dropped"
# 	@sleep 1
# 	@$(podman) exec -it bugvault-data createdb  -T template0  -U bugvault_app_user bugvault && echo "created new database"
# 	@sleep 3
# 	@$(podman) stop bugvault-data
...
# psycopg2.OperationalError: FATAL:  database "bugvault" does not exist
# 
# 
...
#   File "/usr/local/lib64/python3.9/site-packages/psycopg2/__init__.py", line 122, in connect
#     conn = _connect(dsn, connection_factory=connection_factory, **kwasync)
# django.db.utils.OperationalError: FATAL:  database "bugvault" does not exist
