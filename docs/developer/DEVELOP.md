...
Instructions on how to setup local dev environment and contribute to bugvault.
...
Before starting bugvault, you need to define environment variables.
...
Before starting bugvault the first time, or when updating to newer images, you need to log in to registries.
...
````
> tox -e tests bugvault/test_endpoints.py::TestEndpoints::test_list_flaws collectors/bzimport/tests/
````
...
````
podman exec -it testrunner tox -e tests -- bugvault/tests/test_endpoints.py::TestEndpoints::test_list_retrieve_multiple_flaws_by_cve_id
````
...
* Helps: Cleaning existing bugvault podman containers and volumes for whatever reason. (Easier and less heavy-handed than `podman rm -a` and similar actions.)
...
* `make shell-service` (runs `manage.py shell` in `bugvault-service` container)
* `make bash-service` (runs bash in `bugvault-service` container)
...
```sh
$ podman logs -f bugvault-service
```
...
If you have new `requirements*.txt` and want to apply them to the local venv **and bugvault containers** (so that you don't need to rebuild images and containers):
...
To restart bugvault after `make build`, run e.g. `make start-local`.
...
If you don't mind losing the database and are experiencing issues, the fastest way to clean podman's bugvault-related state and rebuild the images is:
...
* Debugging build problems that have secondary effects on the database (if an image doesn't work correctly, the DB might not get created correctly, which then makes debugging harder after the build problems are resolved, because bugvault can still crash because of the corrupted database state).
* Debugging build problems when you don't care about database contents (same reasoning as above).


### Debugging ">waiting for bugvault-service"
...
2. What does this return? `podman exec -it bugvault-data pg_isready || echo error`
  ...
  - Look at `podman logs bugvault-data` for more details about the database's state.

3. What does this return? `podman exec -it bugvault-service bash -c "python3 manage.py migrate --settings config.settings_local"`

  - If `no container with name or ID "bugvault-service" found: no such container`, there's a problem in building the bugvault-service container. Most probably, other containers built from `Dockerfile` have the same problem. Run `make build` and carefully investigate the output.
  - If `Error: can only create exec sessions on running containers: container state improper`, there's something that causes the container to stop upon starting. Run `podman logs bugvault-service`.
  - If `django.db.migrations.exceptions`, it's probably just a migration error/collision. You can resolve it outside of the bugvault-service container, using `make stop-local; make start-local-psql; make bash-local` and then doing the appropriate Django-related software development steps.
  - If it seems to run correctly (e.g. `No migrations to apply.`), check `podman logs bugvault-service` why the service doesn't start.
  - If `RuntimeError: populate() isn't reentrant`, it's most probably a broken database.
    ...
    - For more involved debugging, use `podman logs bugvault-service`, `podman logs bugvault-data`, and inside `podman exec -it bugvault-service bash` follow https://stackoverflow.com/a/55929118 to uncover a more specific Django error message.
  - If `django.db.utils.OperationalError: could not translate host name "bugvault-data" to address: Name or service not known`, bugvault-service can't find the hostname `bugvault-data`, either because the container bugvault-data is not running, or because there's a podman network issue that breaks container-to-container communication.
    - If `podman exec -it bugvault-data pg_isready || echo error` returns an error, try to get the `bugvault-data` container up and running first (see previous debugging steps).
    - If bugvault-data is running correctly, it's possible this is an instance of the bug https://bugzilla.redhat.com/show_bug.cgi?id=1980157 (but actually who knows, I [jsvoboda] don't know enough about this :-( ).
...
### Debugging ">waiting for bugvault-data"

If the container bugvault-data doesn't exist, you probably built/rebuilt/compose-down'd your environment and didn't call `make start-local`, so the images were not instantiated into containers.
...
To create `bugvault_data_backup_dump.db.gz`:
...
To **delete the existing database** and restore from the file `bugvault_data_backup_dump.db.gz`:
...
[REST API docs](http://localhost:8000/bugvault/api/v1/schema/swagger-ui/)
...
```bash
$ podman exec -it bugvault-service python3 manage.py flush
```

Note that if the rest of the database gets broken, it's better to delete the whole volume and bugvault-data container, which recreates the whole database with the correct ACLs, users, etc:
...
alternately you may also run the shell directly on bugvault_service container (once it has
been created)

```bash
podman exec -it bugvault-service python3 manage.py shell --settings=config.settings_shell
...
* Mark model field as deprecated with `bugvault.helpers.deprecate_field` which is
...
Before calling any Model function, make sure the ACLs are set correctly:
    ...
    from bugvault.models import Flaw

    acls = [uuid.uuid5(
                uuid.NAMESPACE_URL, "https://bugvault.prod.redhat.com/ns/acls#data-prodsec"
            )]

    stmt = "SET bugvault.acl='%s';"  # Set parameter once before any DB calls
...
You can connect to the database via Django:
    ...
    podman exec -it bugvault-service python3 manage.py dbshell --database bugvault --settings config.settings_shell
    # config.settings_local also works, if unspecified manage.py should default to what bugvault-service already uses

Or directly:

    podman exec -it bugvault-data psql --db bugvault --username=bugvault_app_user or bugvault_admin_user

Once connected, be sure to set bugvault.acl:

    SET bugvault.acl='1a1f73d5-2ada-507c-868f-e9e505140b45';
    -- This is uuid.uuid5(uuid.NAMESPACE_URL, "https://bugvault.prod.redhat.com/ns/acls#data-prodsec")
...
```
    bugvault-service:
      container_name: bugvault-service
      build: .
      image: bugvault-service
      stdin_open: true
      tty: true
      ports:
        - "8000:8000"
      ...
      command: ./scripts/setup-bugvault-service.sh
      volumes:
        - ${PWD}:/opt/app-root/src:z
      depends_on: ["bugvault-data"]
```
...
```
ALTER TABLE bugvault_flaw DISABLE TRIGGER ALL;
ALTER TABLE bugvault_affect DISABLE TRIGGER ALL;
```
...
```
> python3 manage.py pgtrigger disable bugvault.Flaw:insert_insert
```

To disable a trigger:
```
> python3 manage.py pgtrigger enable bugvault.Flaw:insert_insert
```
