...
from bugvault.constants import DATETIME_FMT
...
from bugvault.models.affect import AFFECTEDNESS_VALID_RESOLUTIONS
...
class FlawFactory(BaseFactory):
    ...
    acl_read = factory.LazyAttribute(
        lambda o: (
            [
                uuid.uuid5(
                    uuid.NAMESPACE_URL, f"https://bugvault.prod.redhat.com/ns/acls#{group}"
                )
            ...
            if o.embargoed is False
                ...
                uuid.uuid5(
                    uuid.NAMESPACE_URL,
                    f"https://bugvault.prod.redhat.com/ns/acls#{settings.EMBARGO_READ_GROUP}",
                )
            ]
        )
    )
    acl_write = factory.LazyAttribute(
        lambda o: (
            ...
            if o.embargoed is False
                ...
                uuid.uuid5(
                    uuid.NAMESPACE_URL,
                    f"https://bugvault.prod.redhat.com/ns/acls#{settings.EMBARGO_WRITE_GROUP}",
                )
            ]
        )
    )
