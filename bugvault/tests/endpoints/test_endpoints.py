...
from bugvault.core import generate_acls, set_user_acls
from bugvault.helpers import ensure_list
from bugvault.models import Affect, Flaw, Impact
from bugvault.pagination import HardLimitOffsetPagination
from bugvault.tests.factories import AffectFactory, FlawFactory

pytestmark = pytest.mark.unit


class TestEndpoints(object):
    def test_bugvault_service_health(self, client):
        """test access to bugvault service health endpoint"""
        response = client.get("/bugvault/healthy")
        assert response.status_code == 200

    def test_status(self, auth_client, test_api_uri):
        """test access to bugvault service status endpoint"""

        response = auth_client().get(f"{test_api_uri}/status")
        assert response.status_code == 200
        body = response.json()
        assert body["bugvault_data"]["flaw_count"] == 0

    def test_manifest(self, auth_client, test_api_uri):
        """test access to bugvault package manifest endpoint"""
    ...
    def test_whoami(self, auth_client, root_url):
        res = auth_client().get(f"{root_url}/bugvault/whoami").json()
...
class TestCustomExceptionHandling:
    @pytest.mark.urls("bugvault.tests.urls")
