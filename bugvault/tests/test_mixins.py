...
from bugvault.core import generate_acls
from bugvault.exceptions import DataInconsistencyException
from bugvault.mixins import Alert, AlertMixin
from bugvault.models import (
    Affect,
    Flaw,
    FlawSource,
    Impact,
...
from bugvault.tests.factories import (
    AffectFactory,
    FlawFactory,
    PsModuleFactory,
    PsProductFactory,
...
def get_acl_read():
    return [
        uuid.uuid5(
            uuid.NAMESPACE_URL,
            "https://bugvault.prod.redhat.com/ns/acls#data-prodsec",
        )
    ]
...
def get_acl_write():
    return [
        uuid.uuid5(
            uuid.NAMESPACE_URL,
            "https://bugvault.prod.redhat.com/ns/acls#data-prodsec-write",
        )
    ]
...
class TestMultiMixinIntegration:
    ...
    def test_tracker_validation_bugzilla(
        ...
        assert "bugvault | Flaw" not in validation_counter
        assert validation_counter["bugvault | Tracker"] == 1
    ...
    def test_affect_validation(
        ...
        # bugvault/api_views.py::AffectView:bulk_post triggers flaw save
        assert len(validation_counter) == 2
        assert validation_counter["bugvault | Flaw"] == 1
        assert validation_counter["bugvault | Affect"] == 1
        assert response.status_code == 200
        ...
        assert response.status_code == 200
        # bugvault/api_views.py::AffectView:bulk_put triggers flaw save
        assert len(validation_counter) == 2
        assert validation_counter["bugvault | Flaw"] == 1
        assert validation_counter["bugvault | Affect"] == 1
