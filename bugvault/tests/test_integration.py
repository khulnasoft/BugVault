"""
    Integration tests assume access to running bugvault-service and bugvault-data tiers
...
class TestIntegration(object):
    def test_healthy_with_curl(self, command_curl, live_server):
        ...
        cmd = [
            ...
            f"{live_server.url}/bugvault/healthy",
        ]
    ...
    def test_status_with_curl(
        ...
        cmd = [
            ...
            "-H",
            "Content-type: application/json",
            f"{live_server.url}/bugvault/api/v1/status",
        ]
        curl_result = subprocess.run(
            cmd, stderr=subprocess.PIPE, stdout=subprocess.PIPE
        )
    ...
    def test_flaws_with_curl(
        ...
        cmd = [
            ...
            "-H",
            "Content-type: application/json",
            f"{live_server.url}/bugvault/api/v1/flaws",
        ]
        curl_result = subprocess.run(
            cmd, stderr=subprocess.PIPE, stdout=subprocess.PIPE
        )
