...
def generate_alertmixins(apps, model):
    Alert = apps.get_model("bugvault", "Alert")
...
def forwards_func(apps, schema_editor):
    ...
    for model_name in ALERTABLE_MODELS:
        model = apps.get_model("bugvault", model_name)
...
class Migration(migrations.Migration):
    dependencies = [
        ...
        ("bugvault", "0126_flaw_default_workflow_state"),
    ]
    ...
    operations = [
        ...
        migrations.RunSQL(
            ...
            sql="""
ALTER TABLE bugvault_alert ENABLE ROW LEVEL SECURITY;
ALTER TABLE bugvault_alert FORCE ROW LEVEL SECURITY;
--following policies define fine grained read/write control on bugvault_alert entity
--policy for entity insert (eg. create)
DROP policy if exists acl_policy_alert_create on bugvault_alert;
...
DROP policy if exists acl_policy_alert_select on bugvault_alert;
...
DROP policy if exists acl_policy_alert_update on bugvault_alert;
...
DROP policy if exists acl_policy_alert_delete on bugvault_alert;
