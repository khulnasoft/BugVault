...
function poormans_pg_isready() {
    timeout -k 1 -s 9 5 bash -c 'echo -n > /dev/tcp/bugvault-data/5432' >/dev/null 2>&1
}

echo "Waiting for bugvault-data:5432 to become available"
...
echo "Waiting for http://bugvault-service:8000/bugvault/healthy to become available"
while ! ( { curl -f http://bugvault-service:8000/bugvault/healthy >/dev/null 2>&1 || exit 1 ; } ) ; do echo -n "." ; sleep 2 ; done
echo
